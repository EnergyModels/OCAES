...
# ----------------
model.E_Wind = Param(model.T)# Wind power generated by farm
model.POE = Param(model.T) # Price of electricity (PJM)

# ----------------
# Variables
# ----------------
# Energy flows
model.E_OCAES = Var(model.T) # Sent to/from OCAES system (in and out)
model.E_Curtail = Var(model.T,within=NonNegativeReals)  # Curtailed energy (out only)
model.E_T = Var(model.T, within=NonNegativeReals)  # Transmitted to shore (in and out)
model.E_Batt = Var(model.T, within=NonNegativeReals)  # Transmitted to shore (in and out)
model.E_Grid = Var(model.T)  # Grid delivered (in and out)

# Air stored
model.m_air = Var(model.T, within=NonNegativeReals) # mass of air stored

# OCAES storage system
model.n_w = Var(within=NonNegativeIntegers)  # Number of wells
model.X_OCAES = Var(within=NonNegativeReals)  # The machinery rating (MW)

# ----------------
# Constraints
# ----------------
model.energy_balance_node1 = Constraint(model.T, rule=energy_balance_node1) # Hourly energy balance - node 1 (offshore)
model.energy_balance_node2 = Constraint(model.T, rule=energy_balance_node2) # Hourly energy balance - node 2 (onshore)
model.reservoir_limit = Constraint(model.T, rule=reservoir_limit)      # Storage reservoir level
#model.w_send = Constraint(model.T, rule=w_send_rule)            # Air storage

model.OCAES_machinery = Constraint(model.T, rule=OCAES_machinery)    # OCAES machinery limits


# ----------------
# Constraint functions
# ----------------
def energy_balance_node1(model, t):
    return model.E_Wind[t] + model.E_OCAES[t]  == model.E_Curtail[t] + model.E_T[t]

def energy_balance_node2(model, t):
    return model.E_T[t] + model.E_Batt[t] == model.E_Grid[t]

def reservoir_limit(model, t):
    if t == 1:
        return model.s[t] == s0 + etaC * model.store[t] - 1 / (etaE) * model.dispatch[t]
    else:
        return model.s[t] == model.s[t - 1] + etaC * model.store[t] - 1 / (etaE) * model.dispatch[t]

def OCAES_dispatch(model, t):
    return model.E_OCAES[t] - model.nl * Xe <= 0

def OCAES_storage(model, t):
    return model.E_OCAES[t] - model.nl * Xe <= 0

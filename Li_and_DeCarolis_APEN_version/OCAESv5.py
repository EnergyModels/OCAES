...
# Objective funtion, the simplified version
def OBJ_rule(model):
...
model.Xs		= Var(within = NonNegativeReals) # The capacity of storage reservoir, with constriant f-2
model.store		= Var(model.T, within = NonNegativeReals) # Operation store(t), MWh/h, with constraint c-1
model.dispatch	= Var(model.T, within = NonNegativeReals) # Operation dispatch(t), MWh/h, with constriant d-1
model.drop		= Var(model.T, within = NonNegativeReals) # Operation drop(t), MWh/h, with constraint e
...
model.reservoir		= Constraint(model.T, rule = reservoir_rule)
model.w_send		= Constraint(model.T, rule = w_send_rule)
...
model.CSTNstore2	= Constraint(model.T, rule = CSTNstore2_rule)
model.CSTNdispatch2	= Constraint(model.T, rule = CSTNdispatch2_rule)
model.CSTNg			= Constraint(rule = CSTNg_rule)
